...
let newArr = localStorage.getItem("newKey");
let getArr;
if (!newArr) {
  const taskarr = new Array();
  localStorage.setItem("newKey", JSON.stringify(taskarr));
  newArr = localStorage.getItem("newKey");
  getArr = JSON.parse(newArr);
}
else{
  getArr = JSON.parse(newArr);
}
...
submit.addEventListener("click", () => {
  if (task.value == "") {
    ...
    getArr.push(value);
    console.log(getArr);
    localStorage.setItem("newKey", JSON.stringify(getArr));
    display();
    task.value = "";
  }
});
